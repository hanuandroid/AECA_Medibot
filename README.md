...
statements, a destructive-SQL repair loop, and — with a live LLM (`gpt-4o-mini`) — eight analytical
questions whose generated SQL must return the same value as hand-written reference SQL on the real
database. All eight pass. Actual output of `run_sql_rag` (SQL shown on one line):
...
| How many billing claims were escalated last month? | `SELECT COUNT(*) AS escalated_claims_last_month FROM claims WHERE status = 'escalated' AND submitted_date >= '2024-11-01' AND submitted_date < '2024-12-01'` | 0 | No billing claims were escalated last month. (as-of 2024-12-28 → November 2024; the latest escalation is 2024-10-24) |
| How many claims were submitted last month? | `SELECT COUNT(*) AS claims_submitted_last_month FROM claims WHERE submitted_date >= '2024-11-01' AND submitted_date < '2024-12-01'` | 9 | A total of 9 claims were submitted last month, specifically in November 2024. |
| How many maintenance tickets were raised this month? | `SELECT COUNT(*) AS total_tickets_raised_this_month FROM maintenance_tickets WHERE raised_date >= '2024-12-01' AND raised_date < '2025-01-01'` | 7 | A total of 7 maintenance tickets were raised this month. |

**Relative dates.** Asked to do calendar arithmetic itself, the LLM once read "last month" as the
as-of month (December). The app now computes the exact ranges (today, this/last month, this/last
year, last 30 days) from the as-of date in Python and puts them in the prompt; the two
"submitted last month" / "raised this month" questions have different answers for November and
December, so a regression is caught by the live tests.
